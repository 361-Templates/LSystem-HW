...
class LSystemAnimator : public emp::web::Animate {
 ...
 public:
  ...
  void SetupGrammar() {
    // =========================================================================
    // TODO 1: Multi-State Grammar Design
    // =========================================================================
    axiom = "A";
    rules['A'] = "A";
  ...
  TissueProperties GetTissueProperties(int branch_depth, int current_depth) {
    // =========================================================================
    // TODO 2: Tissue Maturation & Trait Mapping
    // =========================================================================
    
    // Default placeholder implementation
    double calculated_width = 1;
    std::string line_color = "green";

    return {line_color, calculated_width};
  }

  void DrawSpecializedSymbol(char symbol, double x, double y, double angle, int branch_depth) {
    // =========================================================================
    // TODO 3: Specialized Cell / Organ Rendering
    // =========================================================================
    if (symbol == 'A') {
      // Active apical growth tip: bright green cell cap
      canvas.Circle(x, y, 3.0, "#aeea00", "#76ff03");
    } 
  ...
  void DrawLSystem() {
    ...
    for (char c : current_string) {
      if (c == 'F') {
        ...
        // Calls custom tissue property mapper
        TissueProperties tissue = GetTissueProperties(branch_depth, current_depth);

        canvas.Line(cur_x, cur_y, next_x, next_y, tissue.color, tissue.width);
        ...
        // Hand off all non-movement symbols ('A', 'L', 'B', etc.) to specialized renderer
        DrawSpecializedSymbol(c, cur_x, cur_y, cur_angle, branch_depth);
      }
    }
  }
